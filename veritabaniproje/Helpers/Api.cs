...
namespace veritabaniproje.Helpers
{
    ...
    public class Api
    {
        ...
        private string _urlDB = "Server=localhost;Port=5433;User Id=postgres;Password=1234;Database=HastaneTakipDB";
        ...
        public List<Randevu> GetAllUsersRandevu()
        {
            var list = new List<Randevu>();
            ...
            {
                conn.Open();
                string q1 = "SELECT * FROM \"public\".\"Randevu\"";
                using (var cmd = new NpgsqlCommand(q1, conn))
                {
                    var reader = cmd.ExecuteReader();
                    while (reader.Read())
